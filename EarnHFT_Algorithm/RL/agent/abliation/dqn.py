...
import copy
from RL.util.graph import get_test_contrast_curve, get_sample_curve
import re
...
# trading setting
# 选择这个chunk和GALA的原因 波动大 而且涨跌都有 效果比较明显
parser.add_argument(
    "--train_data_path",
    type=str,
    default="data/GALAUSDT/train/df_15.feather",
    help="training data chunk",
)
parser.add_argument(
    "--dataset_name",
    type=str,
    default="GALAUSDT_df_15",
    help="training data chunk",
)
...
# perfect_trans
parser.add_argument("--perfect_trans", default=False,type=bool)
...
class DQN(object):
    def __init__(self, args):
        ...
        # log path
        if args.perfect_trans:
            self.model_path = os.path.join(
                args.result_path,
                args.dataset_name,
                "ada_{}_perfect_{}".format(args.ada_init, args.perfect_trans),
            ...
            self.model_path = os.path.join(
                args.result_path,
                args.dataset_name,
                "ada_{}_perfect_{}".format(args.ada_init, "false"),
                "seed_{}".format(self.seed),
            )
        ...
        # data
        self.test_df = pd.read_feather(args.train_data_path)
        self.train_df = pd.read_feather(args.train_data_path)
        ...
        # perfect trans
        self.perfect_trans = args.perfect_trans
                    ...
                    if (
                        ...
                        if self.update_counter%10==1:
                            epoch_path = os.path.join(
                                self.model_path,
                                "update_counter_{}".format(self.update_counter),
                            )
