...
class env_cqn(gym.Env):
    ...
    def calculate_avaliable_action(self, price_information):
        ...
        current_action = int(
            self.position * (self.action_dim - 1) / self.max_holding_number
        )
        action_upper = int(
        ...
        if position_lower == 0:
            ...
            action_lower = min(
                int(
                    math.ceil(
                        position_lower * (self.action_dim - 1) / self.max_holding_number
                    )
                    + 1e-12
                ),
                action_upper,
    ...
    def reset(self):
        ...
        self.previous_position = (
            self.initial_action * self.max_holding_number / (self.action_dim - 1)
        )
        self.position = (
            self.initial_action * self.max_holding_number / (self.action_dim - 1)
        )
...
if __name__ == "__main__":
    ...
    env = env_cqn(df=df, initial_action=0)
